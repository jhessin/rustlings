...
path = "{}.rs""#,
    ...
    if cmd.status.success() {
      Ok(CompiledExercise { exercise: self, _handle: FileHandle })
    } else {
      clean();
      Err(ExerciseOutput {
  ...
  pub fn state(&self) -> State {
    let mut source_file = File::open(&self.path)
      .expect("We were unable to open the exercise file!");
    ...
    let min_line =
      ((matched_line_index as i32) - (CONTEXT as i32)).max(0) as usize;
...
mod test {
  ...
  fn test_pending_state() {
    ...
    let expected = vec![
      ...
      ContextLine { line: "".to_string(), number: 2, important: false },
      ContextLine {
        line: "// I AM NOT DONE".to_string(),
        number: 3,
        important: true,
      },
      ContextLine { line: "".to_string(), number: 4, important: false },
      ContextLine {
